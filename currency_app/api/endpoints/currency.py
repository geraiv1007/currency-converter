...
from currency_app.core.dependency import get_currency_service, validate_access_token
...
async def get_live_exchange_rate(
    ...
    exchange_rate = await currency_service.get_live_currency_exchange_rate(
        currency_data,
        email=email
    )
...
async def get_hist_exchange_rate(
    ...
    exchange_rate = await currency_service.get_hist_currency_exchange_rate(
        currency_data,
        email=email
    )
    return exchange_rate
...
async def get_exchange_rate_dynamics(
    ...
    dynamics_data = await currency_service.get_exchange_rate_dynamics(
        currency_data,
        email=email
    )
...
async def get_daily_exchange_rate(
    ...
    daily_data = await currency_service.get_daily_currency_exchange_rate(
        currency_data,
        email=email
    )
    return daily_data
